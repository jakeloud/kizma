#!/usr/bin/env node
var k=Object.create;var F=Object.defineProperty;var M=Object.getOwnPropertyDescriptor;var L=Object.getOwnPropertyNames;var _=Object.getPrototypeOf,R=Object.prototype.hasOwnProperty;var B=(t,e,n,r)=>{if(e&&typeof e=="object"||typeof e=="function")for(let o of L(e))!R.call(t,o)&&o!==n&&F(t,o,{get:()=>e[o],enumerable:!(r=M(e,o))||r.enumerable});return t};var N=(t,e,n)=>(n=t!=null?k(_(t)):{},B(e||!t||!t.__esModule?F(n,"default",{value:t,enumerable:!0}):n,t));var u={backend:{content:`const http = require('node:http')\r
...
`}};var p=require("node:fs/promises"),d=require("node:path"),x=N(require("esbuild"),1);var T=N(require("typescript"),1),J=(t,e)=>t.flags&T.default.SymbolFlags.Alias?e.getAliasedSymbol(t):t,K=(t,e)=>({parameterName:t.name,typeName:e.typeToString(e.getTypeOfSymbolAtLocation(t,t.valueDeclaration))}),U=(t,e)=>t.getCallSignatures()[0].getParameters().map(r=>K(r,e)),Z=(t,e)=>{let n=e.getTypeOfSymbolAtLocation(t,t.declarations[0]);return{functionName:t.name,description:T.default.displayPartsToString(t.getDocumentationComment(e)),parameters:U(n,e)}},z=t=>{let e=T.default.createProgram([t],{}),n=e.getSourceFile(t),r=e.getTypeChecker(),o;try{o=r.getExportsOfModule(r.getSymbolAtLocation(n))}catch{return null}let i=o.find(c=>c.escapedName==="default");if(!i)return null;let s=J(i,r);return Z(s,r)},h=z;var W={string:"text",number:"number"},Y=t=>{let e=t.parameters.map(r=>{let o=r.parameterName,i=W[r.typeName],s=`<input name=${r.parameterName} id="${o}" type="${i}"></input>`;return`<label for="${o}">${r.parameterName}${s}</label>`}),n=`<button>${t.functionName}</button>`;return`<form id="${t.formId}">${e.join("")}${n}</form>`},q=t=>{let{filePath:e,...n}=t;return`<script>${`window.KIZMA_DATA = ${JSON.stringify(n)}`}</script>`},G=({templates:t,schema:e})=>{let n=e.apiPath,r=e.formId;return t.module.content.replace("%TITLE%",e.moduleName).replaceAll("%DESCRIPTION%",e.description).replace("%FORM%",Y(e)).replace("%KIZMA%",q(e))},v=G;var S=require("node:fs/promises"),g=require("node:path"),E=async t=>{try{await(0,S.stat)((0,g.resolve)(t))}catch{return[]}let e=await(0,S.readdir)((0,g.resolve)(t)),n=o=>(async()=>{let s=(0,g.resolve)(t,o),a=(0,g.parse)(s).name;return(await(0,S.stat)(s)).isDirectory()?(await E(s)).map(({relativePath:f,...m})=>({relativePath:[o,f].join("/"),...m})):{relativePath:a,filePath:s,fileName:a}})();return(await Promise.all(e.map(n))).flat()},Q=async t=>await E(t),A=Q;var C=require("node:fs/promises");var V=async t=>{let e=await A(t),r=(await Promise.all(e.map(({filePath:o,relativePath:i})=>new Promise((s,a)=>{(0,C.readFile)(o,"utf-8").then(c=>s({relativePath:i,content:c}),c=>a(c))})))).reduce((o,{relativePath:i,content:s})=>Object.assign(o,{[i]:{content:s}}),{});return typeof u>"u"?r:Object.assign(u,r)},I=V;var X=({templates:t,schemas:e,inDir:n,outDir:r})=>{let o=e.map(({safeFullModuleName:a,relativePath:c})=>`import ${a} from './${n}/${c}'`).join(`
`),i=e.map(a=>{let c=a.relativePath,l=a.safeFullModuleName,f=a.apiPath,m=JSON.stringify(a);return`'${f}':{fn:${l}, schema: ${m}}`}).join(`,
`);return t.backend.content.replace("/*IMPORTS*/",o).replace("/*HANDLERS*/",i).replace("%STATIC_DIR%",r)},j=X;var tt=async(t,e,n,r)=>{let o=await A(t),i=await I(n),s=o.map(({filePath:c,relativePath:l,fileName:f})=>{let m=h(c),w=l.replaceAll(/(\/|\.)/g,"_"),b={filePath:c,relativePath:l,safeFullModuleName:w,moduleName:f,apiPath:`/${t}/${l}`,formId:`/${t}/${l}`,outFile:(0,d.resolve)(e,`${l}.html`)};return Object.assign(m,b)});if(r){let c=(0,d.resolve)(e);await(0,p.rm)(c,{force:!0,recursive:!0}),await(0,p.mkdir)(c,{recursive:!0});let l=s.map(m=>(async()=>{let b=v({templates:i,schema:m}),P=m.outFile,y=(0,d.parse)(P).dir;try{await stat(y)}catch{await(0,p.mkdir)(y,{recursive:!0})}await(0,p.writeFile)(P,b)})()),f=[{templateName:"client",relativeOutFile:"client.js"},{templateName:"styles",relativeOutFile:"styles.css"}].map(({templateName:m,relativeOutFile:w})=>(async()=>{let P=i[m].content,y=(0,d.resolve)(e,w),$=(0,d.parse)(y).dir;try{await stat($)}catch{await(0,p.mkdir)($,{recursive:!0})}await(0,p.writeFile)(y,P)})());await Promise.all(l.concat(f))}let a=j({templates:i,schemas:s,inDir:t,outDir:e});return await x.build({stdin:{contents:a,resolveDir:".",sourcefile:"server.ts",loader:"ts"},bundle:!0,platform:"node",outfile:"server.cjs",external:["dotenv"]}),a},D=tt;var et=t=>{let e=[],n=[];for(let r of t)r.startsWith("-")?r.substring(1).split("").forEach(o=>e.push(o)):n.push(r);return{modifiers:e,values:n}},rt=t=>{let e="public",n="templates",r="api",o=et(t.splice(2)),i=o.modifiers.includes("f"),s=o.modifiers.filter(l=>l!=="f"),a=o.values;for(let l of s)switch(l){case"o":e=a.shift();break;case"t":n=a.shift();break;default:throw new Error(`Unknown modifier: -${l}`)}return a[0]&&(r=a[0]),[r,e,n,i]},O=rt;var ot=O(process.argv);D(...ot);
